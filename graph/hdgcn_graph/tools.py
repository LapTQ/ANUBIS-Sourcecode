...
def get_spatial_graph_original(num_node, self_link, inward, outward):
    I = edge2mat(self_link, num_node)       # (num_node, num_node)
    In = normalize_digraph(edge2mat(inward, num_node))  # (num_node, num_node)
    Out = normalize_digraph(edge2mat(outward, num_node))  # (num_node, num_node)
    A = np.stack((I, In, Out))             # (3, num_node, num_node)
    return A
