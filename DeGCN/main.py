...
def get_parser():
    ...
    parser.add_argument(
        '--config',
        default='config/anubis/anubis.yaml',
...
#             raise ValueError()
    ...
    def start(self):
        if self.arg.phase == 'train':
            ...
            self.data_shape = [1, 3, self.arg.train_feeder_args['window_size'], 
                            self.arg.model_args['num_point'], 2]
            
            if any(dim <= 0 for dim in self.data_shape):
                self.print_log('Warning: Invalid data shape detected. Skipping model profile.')
                self.print_log(f'Data shape: {self.data_shape}')
            else:
                try:
                    dummy_input = torch.rand(self.data_shape)
                    model = import_class(self.arg.model)(**self.arg.model_args)
                    flops, params = thop.profile(model, inputs=(dummy_input,), verbose=False)
                    self.print_log('Model profile: {:.2f}G FLOPs and {:.2f}M Parameters'.format(
                        flops / 1e9, params / 1e6))
                except Exception as e:
                    self.print_log(f'Error in model profiling: {str(e)}')
                    self.print_log(f'Data shape: {self.data_shape}')
