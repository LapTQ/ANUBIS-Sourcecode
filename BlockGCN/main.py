    ...
    def load_data(self):
        ...
        if self.arg.phase == 'train':
            ...
            if self.arg.num_worker > 0:
                loader_args['prefetch_factor'] = 16
                
            self.data_loader['train'] = torch.utils.data.DataLoader(**loader_args)
    ...
    def train(self, epoch, save_model=False):
        ...
        for batch_idx, (joint, data, label, index) in enumerate(process):
            ...
            with torch.amp.autocast('cuda', enabled=use_amp):
                ...
                one_hot = F.one_hot(label, num_classes=num_class).cuda(device)
                output, z = self.model(data, one_hot, joint)
