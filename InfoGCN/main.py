...
class Processor():
    ...
    def load_data(self):
        ...
        # 公共参数设置
        feeder_args = {
            'window_size': getattr(self.arg, 'window_size', 300),
            'vel': self.arg.use_vel,
            'normalization': getattr(self.arg, 'normalization', False),
            'debug': self.arg.debug  # 确保debug参数传递到测试集
        }
        ...
        if self.arg.phase == 'train':
            dt = Feeder(
                ...
                sort=self.arg.balanced_sampling,
                **feeder_args
            )
            
            sampler = BS(data_source=dt, args=self.arg) if self.arg.balanced_sampling else None
            self.data_loader['train'] = torch.utils.data.DataLoader(
                dataset=dt,
                sampler=sampler,
                batch_size=self.arg.batch_size,
                shuffle=not self.arg.balanced_sampling,
    ...
    def adjust_learning_rate(self, epoch):
        ...
        if epoch < warmup_epochs:
            lr = self.arg.base_lr * (epoch + 1) / warmup_epochs
        else:
            lr = self.arg.base_lr * (0.95 ** (epoch - warmup_epochs))  
    ...
    def train(self, epoch, save_model=False):
        ...
        for data, y, index in tqdm(self.data_loader['train'], dynamic_ncols=True):
            ...
            loss = (
                0.1 * self.arg.lambda_2 * mmd_loss + 
                0.1 * self.arg.lambda_1 * l2_z_mean + 
                cls_loss  
            )
            # backward
