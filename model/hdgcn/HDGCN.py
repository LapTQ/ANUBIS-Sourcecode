...
from graph.hdgcn_graph.tools import get_groups
...
class EdgeConv(nn.Module):
    def __init__(self, in_channels, out_channels, k, num_nodes=32):
    ...
    def get_graph_feature(self, x, k, idx=None):
        N, C, V = x.size()
        if V == 1:  # 单节点时直接复制自身
            x = x.unsqueeze(-1).repeat(1, 1, 1, k)  # [N, C, V, k]
            return torch.cat([x, x], dim=1)  # [N, 2*C, V, k]
        ...
        feature = feature.view(N, V, k, C)  # 此时 k 已动态调整
        x = repeat(x, 'n v c -> n v k c', k=k)
        return rearrange(torch.cat([feature - x, x], dim=3), 'n v k c -> n c v k')
