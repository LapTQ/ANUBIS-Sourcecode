...
def init_seed(seed):
    ...
    np.random.seed(seed)
    random.seed(seed)
def worker_seed_fn(worker_id, seed):
...
class Processor():
    ...
    def worker_seed_fn(worker_id):
        ...
        torch.cuda.manual_seed_all(seed + worker_id + 1)
    ...
    def load_data(self):
        ...
        worker_init_fn = Processor.worker_seed_fn  # 直接使用静态方法
    ...
    def train(self, epoch, save_model=False, save_training_score=False):
        ...
        for batch_idx, (data, label, index) in enumerate(loader):
            ...
            with torch.no_grad():
                ...
                # 如果标签是 one-hot 编码，使用 torch.argmax 转换为类别索引
                if label.dim() > 1:
                    label = torch.argmax(label, dim=1).long().cuda(self.output_device)
                else:
